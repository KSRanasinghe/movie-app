...
function Home() {
  ...
  useEffect(() => {
    const getInitialMovies = async () => {
      const page1URL = `https://www.omdbapi.com/?s=%22new%22&y=2025&page=1&apikey=${API_KEY}`;
      const page2URL = `https://www.omdbapi.com/?s=%22new%22&y=2025&page=2&apikey=${API_KEY}`;

      try {
        const [res1, res2] = await Promise.all([fetch(page1URL), fetch(page2URL)]);
        const data1 = await res1.json();
        const data2 = await res2.json();

        const combinedSearch = [...data1.Search, ...data2.Search];

        const detailPromise = combinedSearch.map(async (movie) => {
  ...
  return (
    <>
      <Wrapper className='bg-gray-900 pt-24 min-h-dvh'>
